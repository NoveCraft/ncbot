...
module.exports = {
  ...
  async messageRun(message, args) {
    const token = args[0];
    const cantidad = (args.length > 1 && args[1]) || 99;
    ...
    const response = await purgeMessages(message.member, message.channel, "TOKEN", cantidad, token);

    if (typeof response === "number") {
      return channel.safeSend(`Se eliminaron correctamente ${response} mensajes`, 5);
