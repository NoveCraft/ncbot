...
async function warn(issuer, target, reason) {
  ...
  if (typeof response === "boolean") return `${target.user.username} ha sido advertido!`;
  if (response === "BOT_PERM") return `No tengo permiso para advertir a ${target.user.username}`;
  else if (response === "MEMBER_PERM") return `No tienes permiso para advertir a ${target.user.username}`;
  else return `No se pudo advertir a ${target.user.username}`;
}
